...
class ESOINN
  ...
  def add_signal(input_signal)
    ...
    if add_edge? winners[0], winners[1]
      @edges[winners[0]].merge!(winners[1] => 0)
      @edges[winners[1]].merge!(winners[0] => 0)
    else
      @edges[winners[0]].delete winners[1]
      @edges[winners[1]].delete winners[0]
    end
  ...
  def update_weights(winner, input_signal)
    @vertex[winner][:weight].map!.with_index { |x, i| 1 / @vertex[winner][:num_of_signals] * (input_signal[i] - x) }
    @edges[winner].keys.each do |neighbor|
      @vertex[neighbor][:weight].map!.with_index { |x, i| 1 / 100 * @vertex[winner][:num_of_signals] * (input_signal[i] - x) }
    end
  end
  ...
  def delete_old_edges
    @edges.each_value do |neighbors|
      neighbors.each do |neighbor, age|
        neighbors.delete neighbor if age > @max_age
      end
    end
  end
